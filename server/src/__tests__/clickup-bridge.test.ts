...
describeEmbeddedPostgres("clickupBridgeService.pollInbound", () => {
  ...
  it("does not retry active bridges that are still mid-flight", async () => {
    const companyId = randomUUID();
    const agentId = randomUUID();
    ...
    const retried = await clickupBridgeService(db).retryBridge(bridge!.id);
    ...
    expect(refreshed?.status).toBe("waiting_for_agent_reply");
    expect(refreshed?.consecutivePollFailures).toBe(3);
  });
});
